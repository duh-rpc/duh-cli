		...
		for statusCode, response := range pathItem.Post.Responses.Codes.FromOldest() {
			...
			if !hasItems {
				violations = append(violations, Violation{
					Suggestion: "Paginated responses must include an 'items' array and a 'page' object with 'endCursor'",
			...
			if !hasPage {
				violations = append(violations, Violation{
					Suggestion: "Paginated responses must include an 'items' array and a 'page' object with 'endCursor'",
					Message:    "Paginated response must have a 'page' property",
					Location:   location,
					RuleName:   r.Name(),
					Severity:   SeverityError,
				})
				...
				if pageSchema != nil && pageSchema.Properties != nil {
					if _, hasEndCursor := pageSchema.Properties.Get("endCursor"); !hasEndCursor {
						violations = append(violations, Violation{
							Suggestion: "Paginated responses must include an 'items' array and a 'page' object with 'endCursor'",
							Message:    "Paginated response 'page' must contain 'endCursor' property",
							Location:   location,
							RuleName:   r.Name(),
							Severity:   SeverityError,
						})
