...
components:
  schemas:
	...
	assert.Equal(t, 0, exitCode)
	assert.NotContains(t, stdout.String(), "DESCRIPTION_REQUIRED")
	assert.NotContains(t, stdout.String(), "SUCCESS_RESPONSE")
}

func TestIgnoreUnknownRule(t *testing.T) {
	// Spec with x-duh-lint-ignore containing a nonexistent rule name.
	// The NO_NULLABLE violation should still fire since NONEXISTENT_RULE is not a real rule.
	spec := `openapi: 3.0.0
info:
  title: Test
  version: 1.0.0
...
components:
  schemas:
	...
	assert.Contains(t, stdout.String(), "NO_NULLABLE")
}
