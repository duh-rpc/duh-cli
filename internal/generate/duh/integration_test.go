...
`
	...
	replaceCmd := exec.Command("go", "mod", "edit", "-replace", "github.com/duh-rpc/duh.go/v2=github.com/duh-rpc/duh.go/v2@v2.0.0")
...
func TestGeneratedCodeCompilesWithListOps(t *testing.T) {
	tempDir := t.TempDir()
	require.NoError(t, os.Chdir(tempDir))
	require.NoError(t, os.WriteFile("go.mod", []byte("module github.com/example/test\n\ngo 1.24\n"), 0644))
	...
	assert.Contains(t, clientStr, "duh.NewIterator")
	assert.Contains(t, clientStr, "*duh.Iterator")
	assert.NotContains(t, clientStr, "FetcherName")
	assert.NotContains(t, clientStr, "GenericIterator")
	assert.NotContains(t, clientStr, "FetchPage")
